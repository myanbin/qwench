...
function view() {
	...
	$template->set('updated',$result['updated']);
...
function comment(id) {
	$("#comment_"+id).html("<textarea class=\"commenttextarea\" style=\"margin:0px\" id='commenttext_"+id+"'></textarea><input class=\"smallbutton\" type=\"submit\" value=\"添加评论\" style=\"position:absolute;bottom:0px;margin:0px 10px\" onclick=\"addcomment('"+id+"')\"/>");
}
...
function addcomment(id) {
		...
		if (comment.length < 5) {
			$.fancyalert('你的评论太短，请输入至少5个字');
			return;
		}

		if (comment.length > 600) {
			$.fancyalert('你的评论太长，请输入至多600个字');
			return;
		}
...
function index() {
	...
	if (!empty($searchstringoriginal)) {
		...
		$conditionspost .= " MATCH(title, description) AGAINST ('".escape($searchstring)."' with query expansion) and ";
		$conditionsselect .= ",MATCH(title, description) AGAINST ('".escape($searchstring)."') AS score  ";
		$extratitle = "包含关键字 ".$searchstring;
		if ($defaultorder == 1) {
			$orderby = 'relevance';
			$order = 'score desc';
		}
	...
	if (!empty($_GET['tag'])) {
		...
		$extratitle = "被加注标签 ".createSlug($_GET['tag']);
	}
